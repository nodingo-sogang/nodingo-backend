...
#### C-1. `ZSET`(Sorted Set) + `HASH`기반 실시간 주간 리더보드 엔진

* 유저가 퀴즈 정답, 노드 탐험, 키워드 스크랩, 출석체크 등의 게임 액션을 달성할 때마다 @TransactionalEventListener(AFTER_COMMIT)로 UserXpChangedEvent를 구독하여 DB 커밋 이후에만 Redis ZSET을 갱신하고 HASH 캐시를 무효화하여 데이터 정합성을 보장합니다.
* `scope == PERSONA`: reverseRangeWithScores로 이미 정렬된 ZSET에서 TOP 10을 O(log N)으로 추출하고, 유저 상세 정보는 HASH multiGet으로 단 1번의 네트워크 I/O로 일괄 조회하여 N+1 문제를 제거합니다.
* `scope == FRIENDS`: 친구 네트워크 ID 목록과 결합하여 Java 메모리 단에서 정렬 컷 연산을 수행합니다.
* `reverseRank` 연산으로 10위권 밖에서도 내 등수를 O(log N)으로 조회하여 하단 고정 UI에 바인딩하며, Redis 장애 시 DB COUNT 쿼리로 graceful degradation합니다.
* `Cache Miss` 발생 시 DB에서 선별 조회 후 HASH에 즉시 적재하는 Cache-Aside 패턴으로 운영합니다.
...
- **SizeAndTimeBasedRollingPolicy**: 10MB 초과 or 매일 자정 롤링
- **AsyncAppender**: I/O 블로킹 없이 비동기 로그 기록
- **Docker Volume Mount**: EC2 호스트와 컨테이너 로그 폴더 동기화

### 로그 저장 경로

| 환경 | 경로 |
|------|------|
| 컨테이너 내부 | `/logs/nodingo-backend.log` |
| EC2 호스트 | `/home/ubuntu/logs/nodingo-backend.log` |
| 압축 아카이브 | `/home/ubuntu/logs/archived/nodingo-backend-{날짜}.{인덱스}.log.gz` |

### 롤링 정책

| 항목 | 값 |
|------|----|
| 롤링 조건 | 매일 자정 **또는** 파일 크기 **10MB** 초과 |
| 압축 포맷 | `.gz` (자동) |
| 보관 기간 | **30일** |
| 최대 전체 용량 | **3GB** |
...
| 상황 | 레벨 | 예시 |
|------|------|------|
| 정상 흐름 | `INFO` | 배치 시작/완료, AI 호출 성공 |
| 데이터 이슈 | `WARN` | 뉴스 없음, 후보 키워드 없음 |
| AI 429 에러 | `ERROR` | RateLimit 초과 |
| 일반 예외 | `ERROR` | 스택트레이스 포함 |
| 정상 스킵 | `DEBUG` | 중복 URI, 캐시 히트 |